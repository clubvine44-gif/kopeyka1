/* KOPEYKA CLOUD v10 — account UI + cloud sync (normalize, no silent overwrite) */
(function(){'use strict';
...
function status(text,good){
  ...
  if(good) status._t=setTimeout(()=>{if(x.textContent===text)x.style.display='none'},4000);
}
...
async function saveCloud(force){
  ...
  try{
    ...
    const c=client();
    let r=await c.rpc('save_user_finance_state',{p_state:payload,p_version:10});
    if(r.error){
...
async function boot(){
  ...
  try{
    ...
    if(session.data.session){
      ...
      status('войдите в аккаунт',false);
      setSyncLabel('Не выполнен вход');
      auth();
    }
    ...
    accountUI();
    status('облако недоступно — работаем локально',false);
    setSyncLabel('Офлайн: '+(e.message||'ошибка'));
    try{authMsg('Ошибка подключения к облаку: '+(e.message||''))}catch(_){}
  }
}
