/* KOPEYKA UI + FINANCE MODEL v4.3 — available-now + target-date reserves by month */
(function(){'use strict';
...
function expenseTotals(s,e){
  let a=0,p=0,ao=0,po=0;const t=today();
  arr(STATE&&STATE.expenses).forEach(x=>{
    if(!range(x.date,s,e))return;
    const v=n(x.amount);
    if(x.mandatory){String(x.date)<=t?ao+=v:po+=v}
    else{String(x.date)<=t?a+=v:p+=v}
  });
  arr(STATE&&STATE.recurring).filter(r=>r&&r.active).forEach(r=>{
    try{
      if(typeof occurrencesInRange==='function'){
        occurrencesInRange(r,s,e).forEach(d=>{
          const v=n(r.amount);
          String(d)<=t?ao+=v:po+=v;
        });
      }
    }catch(_){}
  });
  return{actual:a+ao,planned:p+po,actualRegular:a,plannedRegular:p,actualObligatory:ao,plannedObligatory:po};
}

/* unpaidAll — все непогашенные долги; unpaidInPeriod — срок в текущем периоде */
function debtTotals(s,e){
  let unpaidInPeriod=0,unpaidAll=0,paid=0;
  arr(STATE&&STATE.debts).forEach(d=>{
    const amount=n(d.amount),p=n(d.paid),left=Math.max(0,amount-p);
    if(left>0){
      unpaidAll+=left;
      if(range(d.dueDate||e,s,e))unpaidInPeriod+=left;
    }
    if(p>0&&d.paidDate&&range(d.paidDate,s,e))paid+=Math.min(p,amount);
  });
  return{unpaid:unpaidInPeriod,unpaidAll,paid};
}

/*
  Резервы:
  - fixed / percent — как раньше, на текущий период
  - target + targetDate:
      * если дата цели внутри периода [s,e] → учитываем весь остаток (left)
      * если дата цели после конца периода (напр. след. месяц) → 0 в этом месяце
      * если дата уже прошла и есть leftover → учитываем в текущем периоде (просроченная цель)
*/
function reserveNeed(s,e,totalIncome){
  let due=0;const t=today();
  arr(STATE&&STATE.reserves).filter(r=>r&&r.active).forEach(r=>{
    let v=0;
    if(r.method==='fixed'){
      v=n(r.fixedAmount);
    }else if(r.method==='percent'){
      v=totalIncome*n(r.percent)/100;
    }else if(r.method==='target'&&r.targetDate){
      const left=Math.max(0,n(r.target)-n(r.saved));
      if(left>0){
        const td=String(r.targetDate);
        if(td>=String(s)&&td<=String(e)){
          // цель в этом месяце/периоде — берём весь остаток
          v=left;
        }else if(td<String(s)){
          // просроченная цель — всё ещё нужно отложить
          v=left;
        }
        // td > e → следующий месяц и позже — в текущем периоде 0
      }
    }
    const dep=arr(STATE&&STATE.reserveOps)
      .filter(o=>o&&o.reserveId===r.id&&o.type==='deposit'&&range(o.date,s,e)&&String(o.date)<=t)
      .reduce((a,o)=>a+n(o.amount),0);
    due+=Math.max(0,Math.round(v)-dep);
  });
  return due;
}
...
function model(state,s,e){
  ...
  // Касса «на руках» по факту за период
  const cash=base+inc.got-ex.actual-db.paid-dep+wd;

  // Доступно сейчас = касса
  //   − все непогашенные долги (не только с сроком в этом месяце)
  //   − будущие обязательные и обычные расходы периода
  //   − резервы, которые относятся именно к этому периоду
  const strict=cash-ex.planned-db.unpaidAll-res;
  const forecast=cash+inc.exp-ex.planned-db.unpaidAll-res;

  const start=String(t)>String(s)?t:s;
  const days=String(e)<String(t)?0:(typeof diffDays==='function'?diffDays(start,e):0)+1;
  const available=Math.max(0,strict);

  return{
    periodStart:s,periodEnd:e,remainingDays:days,
    actualShiftIncome:0,expectedShiftIncome:0,shiftsList:[],
    actualIncome:inc.got,expectedIncome:inc.exp,totalIncome:inc.got+inc.exp,
    actualRegular:ex.actualRegular,plannedRegular:ex.plannedRegular,totalRegular:ex.actualRegular+ex.plannedRegular,
    actualObligatory:ex.actualObligatory,plannedObligatory:ex.plannedObligatory,totalObligatory:ex.actualObligatory+ex.plannedObligatory,
    currentBalance:cash,
    availableNow:available,
    forecastBeforeReserves:Math.max(0,cash+inc.exp-ex.planned-db.unpaidAll),
    forecastAfterReserves:Math.max(0,forecast),
    strictAvailable:available,
    reservesNeeded:res,
    debtsDue:db.unpaidAll,
    debtsPaid:db.paid,
    reserveDeposits:dep,reserveWithdrawals:wd,
    noReserveLimit:days?Math.max(0,Math.floor((cash+inc.exp-ex.planned-db.unpaidAll)/days)):0,
    safeLimit:days?Math.max(0,Math.floor(forecast/days)):0,
    strictLimit:days?Math.max(0,Math.floor(strict/days)):0
  };
}
...
function renderHomeV4(){
  try{
    ...
    let h='<div class="card hero"><div class="hero-top"><div class="display" style="font-weight:800;font-size:16px;">'+MONTHS_NOM[VIEW_MONTH.m-1]+' '+VIEW_MONTH.y+'</div><div class="period-switch"><button data-act="prevmonth">‹</button><button data-act="nextmonth">›</button></div></div><div class="spend-today"><div class="label">Можно тратить сегодня</div><div class="big-number pos">'+fmtR(s.safeLimit)+'</div><div class="faint">после обязательств, резервов и долгов</div></div></div><div class="grid2">'+statCard('Доступно сейчас',fmtR(s.strictAvailable),'с учётом расходов, обязательных, долгов и резервов периода',[],'')+statCard('Получено',fmtR(s.actualIncome),'введённый тобой доход',[],'income')+'</div><div class="grid2">'+statCard('Ожидается',fmtR(s.expectedIncome),'ещё не получено',[],'income')+statCard('Расходы',fmtR(s.actualRegular+s.actualObligatory),'уже потрачено',[['Будущие',fmtR(s.plannedRegular+s.plannedObligatory)]],'expenses')+'</div><div class="grid2">'+statCard('Резервы',fmtR(saved),'накоплено',[['Нужно отложить',fmtR(s.reservesNeeded)]],'reserves')+statCard('Должен',fmtR(deb.reduce((a,d)=>a+Math.max(0,n(d.amount)-n(d.paid)),0)),'не погашено',[['Погашено',fmtR(s.debtsPaid)]],'reserves')+'</div>';
