...
const copy=()=>Array.isArray(STATE?.income)?JSON.parse(JSON.stringify(STATE.income)):[];
const nonempty=a=>Array.isArray(a)&&a.length>0;
const shadow=()=>{try{const a=copy();if(nonempty(a))localStorage.setItem(KEY,JSON.stringify(a))}catch(_){}};
const restore=()=>{try{const a=JSON.parse(localStorage.getItem(KEY)||'null');if(nonempty(a)&&!nonempty(STATE?.income)){STATE.income=a;return true}}catch(_){}return false};
...
setInterval(()=>{if(STATE&&nonempty(STATE.income))shadow();},250);
})();
